...
def _blog_editor(item: dict, mode: str, wp_base_url: str = "", show_editor: bool = False, gsc_data: dict = None) -> ui.UINode:
    ...
    # ── GSC stats section ────────────────────────────────────────────────────
    gsc_section_children = []
    if gsc_data and gsc_data.get("stats"):
        stats = gsc_data["stats"]
        kws = gsc_data.get("keywords", [])[:8]
        gsc_section_children = [
            ui.Stack(direction="horizontal", gap=16, children=[
                ui.Stat(label="Clicks (90d)",    value=str(stats.get("clicks", 0)),
                        color="blue",   icon="MousePointerClick"),
                ui.Stat(label="Impressions",     value=f"{stats.get('impressions', 0):,}",
                        color="gray",   icon="Eye"),
                ui.Stat(label="CTR",             value=f"{stats.get('ctr', 0):.1f}%",
                        color="green",  icon="TrendingUp"),
                ui.Stat(label="Avg Position",    value=str(stats.get("position", "—")),
                        color="purple", icon="Hash"),
            ]),
            *([ ui.Header(text="Top queries for this page", level=6) ] if kws else []),
            *([ ui.DataTable(
                columns=[
                    ui.DataColumn(key="query",    label="Query",      width="50%"),
                    ui.DataColumn(key="clicks",   label="Clicks",     width="15%"),
                    ui.DataColumn(key="impressions", label="Impr.",   width="15%"),
                    ui.DataColumn(key="position", label="Position",   width="20%"),
                ],
                rows=[{
                    "query":       k.get("query", ""),
                    "clicks":      str(k.get("clicks", 0)),
                    "impressions": str(k.get("impressions", 0)),
                    "position":    str(k.get("position", 0)),
                } for k in kws],
            ) ] if kws else []),
        ]
    elif gsc_data is not None and not gsc_data.get("error") and wp_url:
        gsc_section_children = [
            ui.Text(content="No GSC data yet — page may not have impressions or GSC not connected.", variant="caption"),
        ]

    gsc_section = ui.Section(
        title="📊 Google Search Console",
        collapsible=True,
        children=gsc_section_children or [
            ui.Text(content="Connect GSC in Settings to see organic performance data.", variant="caption"),
        ],
    ) if wp_url else None

    extra = [ui.Divider(), gsc_section] if gsc_section else []
